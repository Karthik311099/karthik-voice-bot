...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadVoices = () => {
      ...
      // Even more aggressive male voice search
      const maleVoice = voices.find(v => 
        (v.name.toLowerCase().includes('male') || v.name.toLowerCase().includes('guy') || 
         v.name.includes('David') || v.name.includes('James') || v.name.includes('Stefan')) && 
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      // Update history without opening sidebar or causing layout shifts
      if (activeSessionId) {
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-full sm:w-80 bg-slate-900 border-r border-slate-800 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <div className="p-6 border-b border-slate-800 flex items-center justify-between">
            ...
            <button onClick={() => setIsSidebarOpen(false)} className="p-2 text-slate-400 hover:text-white bg-slate-800 rounded-lg">
              <X size={24} />
            </button>
          </div>
          
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            <button onClick={startNewChat} className="w-full flex items-center justify-center space-x-2 py-3 px-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-all font-bold mb-4 shadow-lg shadow-indigo-500/10">
              <Plus size={20} />
              <span>New Chat</span>
            </button>

            {history.map((s) => (
              <div key={s.id} onClick={() => loadSession(s)} className={`group relative flex items-center justify-between p-3 rounded-xl border transition-all cursor-pointer ${activeSessionId === s.id ? 'bg-indigo-600/10 border-indigo-500/50 text-indigo-400' : 'bg-slate-800/30 border-transparent text-slate-400 hover:bg-slate-800/50'}`}>
                ...
                <button onClick={(e) => deleteSession(e, s.id)} className="absolute right-2 p-2 hover:text-red-400 rounded-lg transition-all">
                  <Trash2 size={14} />
                </button>
              </div>
            ))}
      ...
      <div className="flex-1 flex flex-col w-full h-full relative">
        ...
        <header className="h-16 border-b border-slate-800 flex items-center justify-between px-6 bg-slate-950/20 backdrop-blur-md z-30">
          <div className="flex items-center space-x-4">
            <button onClick={() => setIsSidebarOpen(true)} className="p-2 text-slate-400 hover:text-white bg-slate-900 rounded-lg">
              <Menu size={24} />
            </button>
            <h1 className="text-lg font-bold text-white tracking-tight">Karthik AI</h1>
          </div>
          <button onClick={clearChat} className="p-2 text-slate-500 hover:text-red-400">
            <Trash2 size={20} />
          </button>
        </header>

        {/* Chat Area - Centered Wrapper */}
        <main className="flex-1 overflow-y-auto px-4 py-6 scrollbar-hide flex flex-col items-center">
            ...
            {currentChat.length === 0 && !isLoading && (
              <div className="flex flex-col items-center justify-center py-20 text-center space-y-6">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm mt-2">I am Karthik's AI persona. Click the mic to start.</p>
                </div>
              </div>
            )}
        ...
        <div className="p-6 bg-slate-950/40 backdrop-blur-xl border-t border-slate-800 flex flex-col items-center">
          ...
          {isListening && (
            <div className="w-full max-w-md p-3 bg-slate-900 border border-slate-800 rounded-xl mb-4 text-center animate-in slide-in-from-bottom-2">
               <p className="text-xs text-indigo-400 font-bold mb-1 uppercase tracking-widest">Listening</p>
               <p className="text-sm text-slate-300 italic">{transcript || "..."}</p>
            </div>
          )}

          <button onClick={toggleListening} disabled={isLoading} className={`w-20 h-20 rounded-full flex items-center justify-center transition-all transform active:scale-90 ${isListening ? 'bg-red-500 shadow-[0_0_30px_rgba(239,68,68,0.4)]' : 'bg-indigo-600 hover:bg-indigo-500 shadow-[0_0_30px_rgba(79,70,229,0.3)]'}`}>
